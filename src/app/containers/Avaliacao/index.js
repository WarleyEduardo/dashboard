...
/* Modulo 33 -  detalhes da avaliação */
	...
	removeAvalicao() {
		...
		if (window.confirm("Você realmente deseja remover essa avaliação?")) {
			...
			this.props.removeAvaliacao(avaliacao._id, produto._id, usuario.loja, (error) => {
				...
				if (error) return this.setState({ aviso: { status: false, msg: error.mensage } })
		...
		return (
			<div className='flex'>
				...
				<div className='flex-1 flex flex-end'>
					<ButtonSimples type='danger' onClick={() => this.removeAvaliacao()} label='Remover' />
				</div>
			</div>
		);
	}
